...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
parameters varrho betta wr alpha_r  alpha_y alpha_pie alpha_A alpha_G alpha_MS alpha_capqual alpha_trend;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%PARAMETERS%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//
...
%
%
%Quasi_Ramsey_Conventional Implementable in form suitable for optimized rule
%
%log((Rn)/(STEADY_STATE(Rn)))=alpha_r*log((Rn(-1))/(STEADY_STATE(Rn)))
%+alpha_pie*log((PIE)/(STEADY_STATE(PIE)))+alpha_y*log(Y/(STEADY_STATE(Y)))
%+alpha_A*log(A/(STEADY_STATE(A)))+alpha_G*log(G/(STEADY_STATE(G)))
%+alpha_MS*log(MS/(STEADY_STATE(MS)))+alpha_capqual*log(capqual/(STEADY_STATE(capqual)))
%+alpha_trend*log((1+Stochg)/(1+g));
%
%Quasi_Ramsey_Conventional Taylor Rule in form suitable for optimized rule
%
log((Rn)/(STEADY_STATE(Rn)))=alpha_r*log((Rn(-1))/(STEADY_STATE(Rn)))
+alpha_pie*log((PIE)/(STEADY_STATE(PIE)))+alpha_y*log((Y/STEADY_STATE(Y))/(YF/STEADY_STATE(YF)))
+alpha_A*log(A/(STEADY_STATE(A)))+alpha_G*log(G/(STEADY_STATE(G)))
+alpha_MS*log(MS/(STEADY_STATE(MS)))+alpha_capqual*log(capqual/(STEADY_STATE(capqual)))
+alpha_trend*log((1+Stochg)/(1+g));
%
...
//----------------------------------------------------------------------------
//
//optwelrule(strvcat('OMEGAscaled'),3,[0 0 0],[1 5 5]);//standard
optwelrule(strvcat('OMEGAscaled'),8,[0 0 0 -1 -1 -1 -1 -1],[1 5 5 1 1 1 1 1]);//Ramsey
//optwelrule(strvcat('OMEGAscaled'),1,[0 ],[5 ]);//price level
