function [HAG,HB,CG,W_red,D_red,C_red,E5,num_optv,nm,mm,i_optv,QQ_red,UU_redT,RR,M_,options_,oo_] = red_form_aces(aa,cc,dd,e2,e5,W,num_inst,nm,mm,M_,options_,oo_,disp_flag)
            ...
            if abs(tempD)<tol % check if mTD=0
               if isreal(temp)
                  ...
                  if isempty(find(all(tem_i<=critval)))&isempty(find(all(tem_i>critval)))
                     if abs(lam(i,i))<1 
                        penalty=penalty+1;
                     else %this is the saddlepath case
                        nm=nm-1; %reduce FL
                        %now find mtrices F T inv(T) and D
                        F=eye(size(aa));
                        %pick the first nonzero in mTnew: j(i) must be
                        %greater than >critval
                        [junk,j,val]=find(mTnew);
                        for i=1:size(j,2)
                           if abs(val(i))>tol & j(i)>critval
                              j=j(i);
                           break
                           end
                        end   
