...
function [ys,check]=NK_Course_Revised_8_F(ys,exe);
global M_
...
rho=0.8; 
x0=[log(rho/(1-rho)),log(hss/(1-hss)),log(0.99/(1-0.99))];
PIE=PIEss;
PIETILDE=PIE^(1-gamp);
 %options = optimset('TolFun',1e-16);
 %options = optimset('MaxIter',10);
[x,fval] =fsolve(@fun_NK_RES_Course,x0,optimset('Display','off'),PIE);
...
H Htilde J Jtilde PIETILDE betta dy pinfobs robs]';
